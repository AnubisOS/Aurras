...
def execute(intent, entities, prompt):
    ...
    for e in entities:
        if e[0] == "datetime":
            ...
            if predicted_time != []:
                ...
                logging.error(
                    f'Provided datetime "{entities["datetime"]}" could not be parsed'
                )
            break
    ...
    if time_delta > 8:
        return {
            "response": "Forecasts can't be predicted this far into the future"
        }  #! break point - can't predict more than 7 days ahead
    ...
    if time_delta == 0:  # special case for today
        ...
        response = {
            "response": f"On {time.strftime(DATE_FORMAT)} it'll be {temperature} degrees with {weather}"
        }
