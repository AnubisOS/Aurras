...
class PluginManager:
    ...
    def __init__(self, plugin_path: str):
        ...
        if not os.path.isdir(plugin_path):
            logging.error("Plugin directory does not exist")
            return
        ...
        for path in plugin_paths:
            basename, extension = os.path.splitext(path)       # home.hushm.aurras.test.plugin 
    ...
    def generate_response(self, intent: str, entities: dict, prompt: str) -> dict:
        """
        Execute all plugins
        ...
        if len(active_plugins) == 0:
            return {
                "response": f"Your intent was {intent} and entities were {entities}"
            }
        ...
        try:
            response = active_plugin.execute(intent, entities, prompt)
        except:
            response = {
                "response": f"An error occurred while trying to execute the {p.NAME} plugin"
            }
